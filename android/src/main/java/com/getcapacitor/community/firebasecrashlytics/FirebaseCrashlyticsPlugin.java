...
public class FirebaseCrashlyticsPlugin extends Plugin {
    ...
    public void setContext(PluginCall call) {
        if (!call.hasOption("key") || !call.hasOption("value")) {
            call.reject(!call.hasOption("key") ? "key property is missing" : "value propery is missing");
            return;
        }
    ...
    public void didCrashDuringPreviousExecution(PluginCall call) {
        call.resolve(
            new JSObject().put("crashed", com.google.firebase.crashlytics.FirebaseCrashlytics.getInstance().didCrashOnPreviousExecution())
        );
    }
